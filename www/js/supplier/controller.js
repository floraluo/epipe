...
// 找回密码验证
.controller('FetchVerificationCtrl', ['$scope', 'httpService', '$ionicHistory', '$state', '$interval', 
	function($scope, httpService,$ionicHistory,$state,$interval){
	$scope.userRegisInfo = {
        phone: '',
        code: '',
    }
	$scope.captcha = {
		text: "获取验证码",		//获取验证码按钮文字
		disabled: false,		//是否禁用获取验证码按钮
		code_is_error: false	//验证码是否错误
		// interval: 40			//禁用按钮之后多长时间解禁
	};
	$scope.interval = 40;
	...
	// 验证表单、验证码
	$scope.supplierFetchCaptcha = function(myform){
		$scope.show_error = true;
        if (myform.$dirty) {
            if (myform.$valid) {
                // 判断验证码是否正确
                httpService.get("/user/checkPhoneToken/"+$scope.userRegisInfo.phone+"/"+$scope.userRegisInfo.code)
                .success(function(data){
                	if(data.status){
                		$state.go("supplier.resetPassword",$scope.userRegisInfo);
                	}else if(data.errMsg == "请填写正确的验证码"){
                		$scope.captcha.code_is_error = true;
                	}
                })
            }
        }
...
// 找回密码
.controller('ResetPasswordCtrl', ['$scope', '$state', '$stateParams', '$ionicHistory', 'httpService', 
	function($scope, $state ,$stateParams, $ionicHistory, httpService){
	$scope.userRegisInfo = {
		password: ""
	}
	...
	$scope.supplierResetPassword = function(){
		httpService.post("/user/resetPassword",{
			phone: $stateParams.phone,
			code: $stateParams.code,
			password: $scope.userRegisInfo.password
		})
		.success(function(data){
			if(data.status){
				$state.go("supplier.login");
